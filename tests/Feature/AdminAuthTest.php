...
class AdminAuthTest extends TestCase
{
    ...
    public function test_admin_dashboard_is_protected_and_uses_the_admin_locale(): void
    {
        ...
        $this->get('/admin')
            ...
            ->assertSeeInOrder(['Tableau de bord', 'Accueil', 'Se déconnecter'])
            ->assertDontSee('Accès rapides')
            ->assertDontSee('Activité récente')
            ->assertDontSee('État de la configuration')
            ->assertDontSee('Aucune activité pour le moment.')
            ->assertDontSee('Bientôt')
            ->assertDontSee('Déplacer le bloc')
            ->assertDontSee('Réduire le bloc')
    ...
    public function test_admin_dashboard_can_render_in_english(): void
    {
        ...
        $this->actingAs($admin)
            ...
            ->assertOk()
            ->assertSee('Dashboard')
            ->assertSeeInOrder(['Dashboard', 'Home', 'Log out'])
            ->assertDontSee('Quick access')
            ->assertDontSee('Library overview')
            ->assertDontSee('Recent activity')
            ->assertDontSee('Setup status')
            ->assertDontSee('No activity yet.')
            ->assertDontSee('Soon')
            ->assertDontSee('Move block')
            ->assertDontSee('Collapse block')
